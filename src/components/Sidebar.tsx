...
export default function Sidebar({ node, onClose }: SidebarProps) {
  ...
  const containerClass = isMobile ? "fixed bottom-0 left-0 right-0 z-30 flex flex-col bg-zinc-950/98 border-t border-white/10 backdrop-blur-md rounded-t-2xl max-h-[65vh]" : "absolute top-0 right-0 h-full w-80 z-20 flex flex-col bg-zinc-950/90 border-l border-white/10 backdrop-blur-md";
    ...
    <>
      ...
      <div className={containerClass}>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {nodeData.seeds.map((seed: string, i: number) => (
                  <button
                    key={i}
                    onClick={() => { onClose(); generateGraph(seed); }}
        ...
        <div className="px-4 py-4 border-t border-white/10">
          {isExpanded ? (
            <p className="text-xs text-zinc-600 text-center">✓ Already explored</p>
          ) : (
            <button
