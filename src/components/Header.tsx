...
export default function Header(){
    ...
    return (
    <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 py-3 bg-zinc-950/80 border-b border-white/10 backdrop-blur-md">

      {/* Left — logo */}
      <button onClick={resetGraph} className="cursor-pointer flex items-center gap-2 hover:opacity-70 transition-opacity shrink-0">
        <Image
      ...
      {/* Center — breadcrumb trail (hidden on mobile) */}
      {!isMobile && (
        <div className="flex items-center gap-1 overflow-x-auto max-w-xl scrollbar-none">
          {history.map((entry, index) => {
            ...
            const colors    = node ? CATEGORY_COLORS[node.data.category] : null;
