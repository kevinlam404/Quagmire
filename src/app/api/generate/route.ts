...
export async function POST(request: Request){
    ...
    const { data: raw, error: parseError} = parseGeminiJson<RawGenerateResponse>(rawText);
    ...
    if(!raw.rootLabel || !Array.isArray(raw.nodes) || !Array.isArray(raw.edges) || raw.nodes.length === 0){
        return NextResponse.json({
            success: false,
